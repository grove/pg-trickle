...
//! - NAT-CALL: `CALL pgtrickle.create_stream_table(...)` syntax works
...
// ── NAT-CALL: CALL procedure syntax ─────────────────────────────────

/// NAT-CALL: Create a stream table via CALL syntax.
#[tokio::test]
async fn test_call_create_stream_table() {
    let db = E2eDb::new().await.with_extension().await;

    db.execute("CREATE TABLE call_src (id INT PRIMARY KEY, val TEXT)")
        .await;
    db.execute("INSERT INTO call_src VALUES (1, 'x'), (2, 'y')")
        .await;

    db.execute(
        "CALL pgtrickle.create_stream_table(\
         'call_st', 'SELECT id, val FROM call_src', 'calculated', 'FULL')",
    )
    .await;

    let count: i64 = db.count("call_st").await;
    assert_eq!(count, 2);
}

/// NAT-CALL: Refresh a stream table via CALL syntax.
#[tokio::test]
async fn test_call_refresh_stream_table() {
    let db = E2eDb::new().await.with_extension().await;

    db.execute("CREATE TABLE call_ref_src (id INT PRIMARY KEY, val TEXT)")
        .await;
    db.execute("INSERT INTO call_ref_src VALUES (1, 'a')").await;

    db.create_st(
        "call_ref_st",
        "SELECT id, val FROM call_ref_src",
        "1m",
        "FULL",
    )
    .await;

    // Insert more data then refresh via CALL
    db.execute("INSERT INTO call_ref_src VALUES (2, 'b')").await;
    db.execute("CALL pgtrickle.refresh_stream_table('call_ref_st')")
        .await;

    let count: i64 = db.count("call_ref_st").await;
    assert_eq!(count, 2);
}

/// NAT-CALL: Drop a stream table via CALL syntax.
#[tokio::test]
async fn test_call_drop_stream_table() {
    let db = E2eDb::new().await.with_extension().await;

    db.execute("CREATE TABLE call_drop_src (id INT PRIMARY KEY, val TEXT)")
        .await;
    db.execute("INSERT INTO call_drop_src VALUES (1, 'z')")
        .await;

    db.create_st(
        "call_drop_st",
        "SELECT id, val FROM call_drop_src",
        "1m",
        "FULL",
    )
    .await;

    db.execute("CALL pgtrickle.drop_stream_table('call_drop_st')")
        .await;

    // Should no longer exist in catalog
    let count: i64 = db
        .query_scalar(
            "SELECT count(*) FROM pgtrickle.pgt_stream_tables WHERE pgt_name = 'call_drop_st'",
        )
        .await;
    assert_eq!(count, 0);
}

/// NAT-CALL: CALL create_stream_table_if_not_exists works.
#[tokio::test]
async fn test_call_create_if_not_exists() {
    let db = E2eDb::new().await.with_extension().await;

    db.execute("CREATE TABLE call_ine_src (id INT PRIMARY KEY, val TEXT)")
        .await;
    db.execute("INSERT INTO call_ine_src VALUES (1, 'a')").await;

    db.execute(
        "CALL pgtrickle.create_stream_table_if_not_exists(\
         'call_ine_st', 'SELECT id, val FROM call_ine_src', 'calculated', 'FULL')",
    )
    .await;

    let count: i64 = db.count("call_ine_st").await;
    assert_eq!(count, 1);
    ...
    let cat_count: i64 = db
        .query_scalar(
            "SELECT count(*) FROM pgtrickle.pgt_stream_tables WHERE pgt_name = 'call_ine_st'",
        )
        .await;
    assert_eq!(cat_count, 1);
}
