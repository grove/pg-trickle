...
- Thin-operator execution-backed coverage now exists for `semi_join`, `anti_join`, `window`, and `scalar_subquery`; `semi_join` / `anti_join` now cover match-gain/loss, simultaneous left/right deltas, and unmatched-left inserts, while the remaining highest-value thin-operator work is nested-source cases for semi/anti joins, partition-move and multi-window cases for window functions, and shared-source / aggregate-backed scalar-subquery cases
