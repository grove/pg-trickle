...
| ID | Feature | Status |
|----|---------|--------|
| EC01-0 | Q15 IMMEDIATE-mode stop-gap | ✅ Done |
| EC01-1 | Fix `__pgt_row_id` hash collision in `join.rs` | ⚠️ Deferred to v0.22.0 (P0; requires coordinated join.rs + refresh.rs changes) |
| EC01-2 | PH-D1 phantom cleanup in `refresh.rs` | ⚠️ Deferred to v0.22.0 (depends on EC01-1) |
| EC01-3 | TPC-H Q07 + Q15 regression gate | ⚠️ Deferred to v0.22.0 (depends on EC01-1) |
| EC01-4 | Multi-cycle phantom proptest | ⚠️ Deferred to v0.22.0 (depends on EC01-1) |
| SAF-1 | Convert `.unwrap()` → `?` in `sublinks.rs` | ✅ Done |
| SAF-2 | Unsafe reduction half-pass (≥40%) | ✅ Done (479→277 blocks, 42% reduction) |
| SAF-3 | `clippy::unwrap_used` lint gate | ✅ Done |
| OP-6 | Volatile function warning / rejection | ✅ Done |
| TEST-1 | Unit tests for `api/helpers.rs` | ✅ Done (25+ tests) |
| TEST-2 | Unit tests for `api/diagnostics.rs` | ✅ Done (20+ tests) |
| TEST-3 | Unit tests for `dvm/parser/rewrites.rs` | ✅ Done (30+ tests) |
| TEST-4 | Parser fuzz target | ✅ Done (`fuzz/fuzz_targets/parser_fuzz.rs`) |
| TEST-5 | Crash-recovery bgworker resilience test | ✅ Done (3 new tests in `resilience_tests.rs`) |
| ARCH-1 | Split `refresh.rs` into 4 sub-modules | ✅ Done (`src/refresh/` with mod.rs + 4 stub files) |
| ARCH-2 | Recursive CTE fallback observability | ✅ Done (`refresh_reason` column + NOTICE log) |
| OPS-1 | Shadow/canary mode for `alter_stream_table` | ✅ Done (SQL: `canary_begin/diff/promote`) |
| OP-2 | Prometheus HTTP endpoint in bgworker | ✅ Done (`pg_trickle.metrics_port` GUC + HTTP server) |
| OP-3 | `pgtrickle.pause_all()` / `resume_all()` | ✅ Done (SQL migration 0.20→0.21) |
| OP-4 | `pgtrickle.refresh_if_stale(name, max_age)` | ✅ Done (SQL migration 0.20→0.21) |
| OP-5 | `pgtrickle.stream_table_definition(name)` | ✅ Done (SQL alias for export_definition) |
| DOC-1 | Performance Tuning Cookbook | ✅ Done (`docs/PERFORMANCE_COOKBOOK.md`) |
